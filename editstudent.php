...
if(isset($_POST['update'])) {
    ...
    $conn = mysqli_connect('localhost','root','VINEETH@9','attendance');
...
$sql1 = "UPDATE `student_details` SET `name`='$name',`gender`='$gender',`email`='$email',`DOB`='$DOB',`contact_no`='$contact',`department`='$department' WHERE id='$update_id'";
 ...
 if (mysqli_query($conn,$sql1))
  {
   ...
   <script>
	  ...
	  if(!alert('student details updated successfully.')){window.location = "admin.php?view_student=view_student";}
   </script>
   <?php
       } 
   else {
    ?>
     <script>
       // window.location = "admin.php?view_student=view_student";
          if(!alert("Can't update student details.Some error occured")){window.location = "admin.php?view_student=view_student";}
